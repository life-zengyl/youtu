var navSelect = '';
function isLogin(token,redirect) {
    var uri = getQueryString('uri');
    if(uri){
        location.href = uri;
    }
    var urlFile = getCurpath(redirect);
    if(urlFile == ''){
        redirect += 'index.html';
    }
    if (!token) {
        if (getQueryString('key') == '') {
            if (is_weixin()) {
                location.href = ApiUrl + '/index.php?act=connect_wx&op=index&redirect=' + base64encode(redirect);
                return;
            }
            //else{
            //    //非微信浏览器场景下跳转到登陆页
            //    if(getCurpath() != 'login'){ //验证当前访问页面是否登陆页面，防止跳转陷入死循环
            //        location.href = WapSiteUrl + '/tmpl/member/login.html?redirect='+base64encode(redirect);
            //        return;
            //    }
            //}
        } else {
            var tokenString = getQueryString('key');
            ...
            addCookie('key', tokenString);
            addCookie('username', uname);
            var key = getCookie('key');
        }
    }
}
function getCurpath(value){
    var a = location.href;
...
$(function () {
    var key = getCookie('key');
    isLogin(key,window.location.href);
...
function loadSeccode() {
    ...
    $.ajax({
        type: 'get',
        url: ApiUrl + "/index.php?act=seccode&op=makecode",//index.php?act=seccode&op=makecodekey
        async: false,
        dataType: 'json',
        success: function (result) {
...
$('#toChatList').click(function () {
    location.href = WapSiteUrl + '/tmpl/member/chat_list.html';
});